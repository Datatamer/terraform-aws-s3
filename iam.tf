locals {
  ...
  ro_paths     = var.read_only_paths == [] ? [var.bucket_name] : var.read_only_paths
...
data "aws_iam_policy_document" "rw_source_policy_doc" {
  count = var.read_write_paths == [] ? 0 : 1
...
data "aws_iam_policy_document" "path_specific_rw_doc" {
  ...
  version     = "2012-10-17"
  source_json = data.aws_iam_policy_document.rw_source_policy_doc[count.index].json

  dynamic "statement" {
    for_each = local.rw_paths_map

    content {
      sid     = "rw_policy_${statement.key}"
      effect  = "Allow"
      actions = var.read_write_actions
      resources = [
        "arn:aws:s3:::${statement.value}",
...
resource "aws_iam_policy" "rw_policy" {
  ...
  policy = data.aws_iam_policy_document.path_specific_rw_doc[count.index].json
}
